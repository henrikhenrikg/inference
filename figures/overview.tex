...
\node[] at (0.45, -1.7) {$\Updownarrow$};



\node[right] at (-1.7,-2.2) {\tiny \textsc{[MASK]} released \textit{Homeland}};
...
\node[] at (0, 0.4) [fontscale=0.1, text=Red] {\tiny Inconsistent Prediction};
...
% Consistent
\node[right] at (2.3,0) {\tiny \textit{Seinfeld} originally aired on \textsc{[MASK]}};
...
\node[] at (4.45, -1.7) {$\Updownarrow$};



\node[right] at (2.3,-2.2) {\tiny \textsc{[MASK]} released \textit{Seinfeld}};
