...
% \enote{hs}{below: why is the notation used here different from the rest ofthe paper?}
...
However, this objective contains a comparison between the output of two argmax operations, making it discrete and discontinuous, and hard to optimize in a gradient-based framework. We instead relax the objective, and require that the predicted \emph{distributions} $Q_n = softmax(f_\theta(P_n))$, rather than the top-1 prediction, be identical to each other. %we use a softer constraint.
...
Since we evaluate our method on unseen relations, we also split the train/test by the relation type (e.g., location-based relations, which are very common
